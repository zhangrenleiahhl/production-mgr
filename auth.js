/* ==========================================================================
   ...
   全站门禁：window.LOGIN_REQUIRED = true 时，未登录的页面会跳登录页。
            ★ 2026-09-23 **已开启**（账号已建齐、登录已真机实测通过）。
            改回 false 可随时关闭，不会锁死现网 —— 万一有页面进不去，先把这行改回 false 再排查。
   受门禁保护的页（10 个）：管理入口、发货计划、订单管理、物料数据库、数据急救、仪表盘、
            生产管理客户端、内部物流大屏、viewer。
   故意**不拦**的页：login.html（登录页本身）、物流门口大屏（门岗/司机要看）、
            司机扫码（司机用）、扫码.html（内部工序页 —— 它自己有 15 人姓名+口令登录，
            再叠一层会把名单里还没建账号的 5 个人锁死）。
   ========================================================================== */
